...
настройки фикстуры. Эта проблема становится хуже, когда вы пишете
несколько тестов с похожими фикстурами. Без помощи от фреймворка тестирования,
нам пришлось бы дублировать код, который устанавливает фикстуру, для каждого
...
показывает, как мы можем написать тесты для ``StackTest`` таким образом, чтобы повторно 
использовалась не сама фикстура , а код, который её создаёт.
...
использовать ``unset()`` для удаления переменных, указывающих на эти объекты
...
если решите эту основную проблему в проектировании, а затем напишете тесты,
...
использующем глобальную переменную, может сломать другой тест.
...
-
  ...
  для создания локальной переменной в текущей области видимости, ссылающейся на глобальную переменную.
...
Аннотацию ``@backupStaticAttributes``, обсуждаемую в
